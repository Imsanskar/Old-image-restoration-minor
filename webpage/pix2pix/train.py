...
if __name__ == "__main__":
	...
	# load csv for data logging
	generator_error_file = "generator_new_data.csv"
	disc_error_file = "discriminator_new_data.csv"
	...
	generator_file = f"saved_models_new_data/generator_{total_generator_epochs - 1}.pth"
	discriminator_file = f"saved_models_new_data/discriminator_{total_discriminator_epochs - 1}.pth"
	...
	for epoch in range(total_discriminator_epochs, total_discriminator_epochs + 2):
		# break
		...
		with open(generator_error_file, "a") as f:
			f.write(f"Generator Error, {np.mean(loss_G_list)}, {epoch}\n")
		with open(disc_error_file, "a") as f:
			f.write(f"Discriminator Error, {np.mean(loss_D_list)}, {epoch}\n")
	
		torch.save(generator.state_dict(), f"saved_models_new_data/generator_{epoch}.pth")
		torch.save(discriminator.state_dict(), f"saved_models_new_data/discriminator_{epoch}.pth")
